...
| **辐角原理** | $\frac{1}{2\pi i} \oint_C \frac{f'}{f} = N - P$ | 计算区域内零点与极点个数之差 |
| **Rouché定理** | 若在 $C$ 上 $\|f\| > \|g\|$，则 $f$ 和 $f+g$ 在 $C$ 内零点数相同 | 确定方程在指定区域内的零点数 |
| **平均值性质** | $f(z_0) = \frac{1}{2\pi} \int_0^{2\pi} f(z_0 + re^{i\theta})\,d\theta$ | 证明最大模原理 |
| **Cauchy估计** | $\|f^{(n)}(z_0)\| \leq \frac{n! M}{R^n}$（其中 $M$ 为 $f$ 在圆盘上的上界） | 证明Liouville定理 |
| **Jordan引理** | $\lim_{R\to\infty} \int_{\Gamma_R} e^{i\alpha z}f(z)\,dz = 0$（$\alpha > 0$, $\Gamma_R$ 为上半圆） | 含三角函数的无穷积分 |
...
设 $f,g$ 在简单闭曲线 $C$ 内部及其上解析，且在 $C$ 上满足 $|g(z)| < |f(z)|$，则 $f$ 与 $f+g$ 在 $C$ 内部的零点个数相同（含重数）。

**直观**：$f$ 是"主项"，$g$ 是"小扰动"，扰动不改变零点个数。

### 辐角原理（Argument Principle）

设 $f$ 在 $C$ 内部亚纯、在 $C$ 上解析且无零点，则

$$\frac{1}{2\pi i}\oint_C \frac{f'(z)}{f(z)}\,dz = N - P = \frac{1}{2\pi}\Delta_C \arg f(z)$$

其中 $N$ = 零点个数（含重数），$P$ = 极点个数（含重数），$\Delta_C \arg f(z)$ 是 $f(z)$ 沿 $C$ 一周的辐角增量。

### 零点计数标准流程

1. **选主项**：把方程 $h(z)=0$ 拆成 $f(z)+g(z)=0$，其中在边界 $C$ 上 $|f(z)| > |g(z)|$ 严格成立
2. **Rouche 判定**：$f$ 与 $f+g$ 零点数相同
3. **数 $f$ 的零点**：$f$ 通常是多项式或简单函数，零点数可直接数（如 $z^5$ 在 $|z|<1$ 内有 5 重零点）
4. **结论**：原方程在 $C$ 内零点数 = $f$ 的零点数

### 典型应用

- **$z^5 + 3z + 1$ 在 $|z|<1$ 内根的个数**：在 $|z|=1$ 上，取 $f=3z$（$|f|=3$），$g=z^5+1$（$|g| \le |z^5|+1 = 2 < 3$），故 $3z$ 与 $z^5+3z+1$ 零点数相同，$3z$ 在 $|z|<1$ 内有 1 个零点（$z=0$），故原式有 **1 个根**。
- **$z^4 + 5z + 2$ 在 $|z|<2$ 内根的个数**：取 $f=z^4$（$|f|=16$），$g=5z+2$（$|g| \le 12 < 16$），故 $z^4$ 与 $z^4+5z+2$ 零点数相同，$z^4$ 在 $|z|<2$ 内有 4 重零点，故有 **4 个根**。

### 易错点

- Rouche 要求**边界上严格** $|g|<|f|$，不是内部
- 零点按**重数**计算
...
若 $f,g$ 在区域 $D$ 内解析，且在 $D$ 内**有聚点的集合**上相等（例如在 $D$ 内的一段弧、一个收敛到 $D$ 内点的点列上相等），则 $f \equiv g$ 在 $D$ 上恒成立。

**推论**：解析函数的零点若在 $D$ 内有聚点，则函数恒为零。这是"解析延拓唯一"的依据。

### 分式线性变换（Möbius 变换）

$$w = \frac{az+b}{cz+d}, \quad ad-bc \ne 0$$

性质：把圆/直线映为圆/直线、保角、保持交比（cross-ratio）不变：
...
| 原区域 | 目标 | 映射 |
|--------|------|------|
| 上半平面 $\mathrm{Im}\,z>0$ | 单位圆盘 $|w|<1$ | $w=\dfrac{z-i}{z+i}$（$i\to0,\ \infty\to1,\ 0\to-1$）|
| 单位圆盘 $|z|<1$ | 上半平面 | $w=\dfrac{i(1-z)}{1+z}$ |
| 单位圆盘 $|z|<1$ | 单位圆盘 | $w=e^{i\theta}\dfrac{z-a}{1-\bar a z}$（$|a|<1$）|

### Riemann 映射定理

任意**非全平面**的单连通区域 $D$，解析共形等价于单位圆盘 $|w|<1$。且对任意 $z_0\in D$，存在唯一映射 $f$ 使 $f(z_0)=0,\ f'(z_0)>0$。
