...
## 使用规则
- 先写清未知函数、自变量、区域、方程、初始/边界条件以及正则性要求；这些条件本身就属于解的一部分。
- 当题目核心是有限差分、有限元、截断误差或稳定性时，转交数值分析处理；纯抽象的算子谱问题则转交泛函分析。
- 得到表达式后，要同时核验 PDE 本身和每一条初边条件，避免只验证内部方程。

## 知识点 1：一阶方程与特征线
- 线性或拟线性 PDE 用特征方程建立不变量；初始曲线必须是非特征的，才通常能给出局部唯一解。
- 参数化回代时要检查 Jacobian 是否退化、是否出现多值解或交叉的特征线。

## 知识点 2：二阶分类与调和函数
- 对 $Au_{xx}+2Bu_{xy}+Cu_{yy}$ 用 $B^2-AC$ 分类，并说明系数和区域。
- Laplace 方程的解要检验调和性；调和函数可用最大值原理、均值性质和边界唯一性，但必须满足相应的正则性。

## 知识点 3：波动、热与分离变量
- 无界一维波动方程用 d'Alembert 公式并核对初位移与初速度；有界区间按边界条件选择正弦或余弦特征函数。
- 热方程的特征展开要说明系数由初值投影得到，并检查边界条件与时间衰减。
- Fourier 级数/Green 函数必须标明定义区间、边界类型和收敛意义。

## 知识点 4：能量法与形式伴随
- 能量估计要保留积分区域、边界项和符号；能量守恒还是衰减取决于边界条件。
- 形式伴随需经过分部积分写清导数转移、系数转置/共轭和边界项，不能只改变符号。
